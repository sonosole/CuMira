...
function TCSReduceOther(r, g, seq, N::Int, T::Int)
    ...
    for n = 1:N
        ...
        @inbounds for t = start:stride:T
            r[seq[s-1],t] += g[s-1,t]  # reduce forground state
            r[seq[s  ],t] += g[s,  t]  # reduce labels' states
            r[seq[s+1],t] += g[s+1,t]  # reduce background state
        end
        sync_threads()
    end
    return nothing
...
function Mira.TCS(p::CuArray{TYPE,2}, seqlabel::Vector{Int}; background::Int=1, foreground::Int=2) where TYPE
    ...
    CUDA.@sync begin
        @async @cuda blocks=1 threads=CuThreads(L) tcsfwd(a, p, seq, L, T);
        @async @cuda blocks=1 threads=CuThreads(L) tcsbwd(b, p, seq, L, T);
    end
